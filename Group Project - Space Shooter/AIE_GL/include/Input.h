...
/*-----------------------------------------------------------------------------
	...
	GetPressedCharacters( );	// returns a list of unicode printable characters (modifier keys are supported)
								// eg if the INPUT_KEY_A button on the keyboard was pressed whilst capslock or shift was on,
	...
	// key:			refers to a unique key identifier    eg: INPUT_KEY_SPACE or INPUT_KEY_ESCAPE etc
	// mouseButton:	refers to a unique button identifier eg: INPUT_MOUSE_BUTTON_LEFT or INPUT_MOUSE_BUTTON_RIGHT
...
//------------------------------------------------------------
// Key Mappings (Must match codes in glfw3.h)
//------------------------------------------------------------
...
public:
	...
	static Input *Instance()			{ return m_pInstance; }

	bool IsKeyDown(int INPUT_KEY_ID);
	bool IsKeyUp(int INPUT_KEY_ID);
	bool WasKeyPressed(int INPUT_KEY_ID);
	bool WasKeyReleased(int INPUT_KEY_ID);

	const std::vector<unsigned int> &GetPressedKeys();
	const std::vector<unsigned int> &GetPressedCharacters();

	bool IsMouseButtonDown(int INPUT_MOUSE_ID);
	bool IsMouseButtonUp(int INPUT_MOUSE_ID);
	bool WasMouseButtonPressed(int INPUT_MOUSE_ID);
	bool WasMouseButtonReleased(int INPUT_MOUSE_ID);
...
protected:
	...
	static void Create()			{ m_pInstance = new Input(); }
